...
def create_parser(args):
    ...
    func_parser.add_argument('--buffer',
                             ...
                             type=int,
                             default=20,
                             help='the buffer for linting command defaulted to 20')
...
class Guardails(GuardrailGlobals):
    ...
    def list_to_str_folders(self, folders=None):
        ...
        LOG.info("list_to_str_folders function returned:%s", str(list_to_str))  # pragma: no mutate
        return list_to_str
    ...
    def parse_jscpd_report_json(self, duplicate_limit, json_file):
        ...
        Parameters:
          duplicate_limit (int): allowed duplicate limit for the jscpd/copy paste detection
          json_file (string): jscpd report file
        ...
        try:
            ...
            if lines > 0:
                if per <= float(duplicate_limit):
                    LOG.info("Guardrail gating passed jscpd")  # pragma: no mutate

                else:
                    LOG.info("Guardrail gating, failed jscpd.")  # pragma: no mutate
                    sys.exit(1)
            else:
                LOG.info("jscpd report not correctly generated")  # pragma: no mutate
                sys.exit(1)
        except KeyError:
            LOG.info("jscpd report not generated, key is not found in json")  # pragma: no mutate
            sys.exit(1)
    ...
    def check_pass_fail(failure, total, allow_fail_per):
        ...
        if total:
            if failure <= per:
                LOG.info("Guardrail gating, passed mutation.")  # pragma: no mutate
            else:
                LOG.info("Guardrail gating, failed mutation.")  # pragma: no mutate
                sys.exit(1)
        else:
            LOG.info("Guardrail gating, failed: mutation test did not run.")  # pragma: no mutate
            sys.exit(1)
    ...
    def get_index_cnn(root):
        ...
        try:
            ...
            LOG.info("Guardrail unable to find the tag CCN in the report ")  # pragma: no mutate
            sys.exit(1)
    ...
    def get_all_func_cnn(self, root):
        ...
        try:
            ...
            for item in functions:
                ...
                temp_val = []
            if not cyclo_dict:
                LOG.info("Guardrail unable to find the tags item/value/name in the report file ")  # pragma: no mutate
                sys.exit(1)
            else:
                return cyclo_dict
        except KeyError:
            LOG.info("Guardrail unable to find the tags item/value/name in the report ")  # pragma: no mutate
            sys.exit(1)
        ...
        try:
            ...
            for functions in root.iter('measure'):
                if functions.attrib['type'] == "Function":
                    LOG.info("successfully found functions with CNN")  # pragma: no mutate
            ...
            LOG.info("tags required are not found in cc.xml report file path")  # pragma: no mutate
            sys.exit(1)
    ...
    def parse_mutmut_report_xml(self, allow_fail, xml_file):
        ...
        try:
            ...
            LOG.info("mutmut.xml report file path cound not be found")  # pragma: no mutate
            sys.exit(1)
    ...
    def guardrail_test(self):
        """ Function which conduct testing using pytest for the folders specified"""
        LOG.info("Started test gate")  # pragma: no mutate
        retval = self.call_subprocess('%s -m pytest %s %s --cov-report "html" --cov=%s' %
                                      (self.python, self.pytest, self.cov_rc_file(), self.list_to_str_folders(
                                          self.src_folder)))
        self.validate_return(retval, "Test execution and coverage generation", False)
        print("Passed testing using pytest")  # pragma: no mutate
        print('====================================')  # pragma: no mutate

    def guardrail_coverage(self):
        """
        Function which conduct coverage based on the test executed using pytest for the folders
        specified
        """
    ...
    def mov_cov_report(self):
        """
        Function which is used to move the coverage report folder from where it generated to specified report folder
        """
    ...
    def guardrail_mutation(self):
        """ Function which is used to conduct the mutation testing and gate it with allowed mutants % """

        LOG.info("Started mutation gate")  # pragma: no mutate
        retval = self.call_subprocess('%s -m mutmut --paths-to-mutate %s run || true' % (self.python, os.path.join(
            self.list_to_str_folders(self.src_folder))))
        self.validate_return(retval, "Mutation testing ", False)
        retval = self.call_subprocess(
            'python -m mutmut junitxml --suspicious-policy=ignore --untested-policy=ignore > %s' % (
                os.path.join(self.report_folder, 'mutmut.xml')))
        self.validate_return(retval, "Mutation testing report generation", False)
        self.parse_mutmut_report_xml(self.allow_mutants, os.path.join(self.report_folder, 'mutmut.xml'))
        print("Passed mutation testing gate")  # pragma: no mutate
        print('====================================')  # pragma: no mutate

    def guardrail_cyclomatic_complexity(self):
        """
        Function which is used to conduct the cyclomatic complexity and gate it with allowed cyclomatic
        complexity value
        """
        LOG.info("Started cyclomatic complexity gate")  # pragma: no mutate
        retval = self.call_subprocess('%s -m lizard %s %s -X > %s' % (self.python, os.path.join(
            self.list_to_str_folders(self.src_folder)), self.get_exclude_cc(),
    ...
    def orchestrate_guardrails(self):
        """ Function which is used to orchestrate all the activities of guardrails """
        self.clean_log()
